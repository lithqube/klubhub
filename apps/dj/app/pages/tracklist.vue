...
const preset = ref<'default' | 'dark' | 'light' | 'neon' | 'minimal'>(
  'default',
);
...
const initializePage = async () => {
  try {
    ...
    visibleFields.value = settings.tracklist_preferences?.visibleFields ?? [
      'title',
      'artist',
      'bpm',
      'key',
    ...
    trackRangeStart.value =
      settings.tracklist_preferences?.trackRangeStart ?? null;
    trackRangeEnd.value = settings.tracklist_preferences?.trackRangeEnd ?? null;
    logoPath.value = settings.logoPath ?? null;
    logoPosition.value =
      settings.tracklist_preferences?.logoPosition ?? 'top-left';
...
const deleteTracklistHandler = async (id: string) => {
  if (
    !confirm(
      `Delete "${tracklist.value?.title}"? This will permanently remove ${tracks.value.length} tracks.`,
    )
  ) {
    return;
  }
  ...
  <div class="min-h-screen bg-gray-950 text-white p-6">
    ...
    <div v-if="step === 'upload'" class="max-w-2xl mx-auto text-center">
      ...
      >
        <div v-if="!uploadFile">
          ...
          >
            <path
              ...
              strokeWidth="2"
              d="M7 16v4a2 2 0 002 2h8a2 2 0 002-2v-4M11 8h6m0 0l4-4m-4 4l4 4m-5-4V4a2 2 0 012-2h1.5a2 2 0 011.414.586l2 2A2 2 0 0115 8.586v7a2 2 0 01-2 2h-2M9 16h6"
            />
          </svg>
          ...
          <button
            ...
            class="mt-2 px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded"
          >
            Select File
          </button>
          <input
        ...
        <div
          v-if="uploadError"
          class="mt-4 p-3 bg-red-900 rounded text-red-300"
        >
          {{ uploadError }}
        </div>
      </div>
    ...
    <div v-else class="flex gap-6 p-6 min-h-screen bg-gray-950 text-white">
      ...
      <div class="flex-1 bg-gray-800 rounded-lg p-4 overflow-hidden">
          ...
          <table class="min-w-full border-collapse">
            <thead>
              <tr class="border-b border-gray-700">
                <th
                  class="text-left py-2 pl-4 text-sm font-medium text-gray-400"
                >
                  #
                </th>
                <th
                ...
                <th
                  v-if="visibleFields.value.includes('bpm')"
                  class="text-left py-2 pl-4 text-sm font-medium text-gray-400"
                >
                  BPM
                </th>
                <th
                  v-if="visibleFields.value.includes('musical_key')"
                  class="text-left py-2 pl-4 text-sm font-medium text-gray-400"
                >
                  Key
                </th>
                <th
                  v-if="visibleFields.value.includes('durationSecs')"
                  class="text-left py-2 pl-4 text-sm font-medium text-gray-400"
                >
                  Duration
                </th>
            ...
            <tbody>
              ...
              >
                  ...
                  <div class="relative w-10 h-10">
                    ...
                    >
                      <template v-if="track.artworkStatus === 'manual'">
                        ...
                        >
                          <span
                            class="bg-black/50 text-xs text-white px-1 rounded"
                            >LOCK</span
                          >
                        </div>
                      </template>
                ...
                >
                  <span
                    v-if="track.bpm !== null && track.bpm !== undefined"
                    class="cursor-pointer hover:underline"
                ...
                >
                  <span
                    v-if="track.musicalKey"
                    class="cursor-pointer hover:underline"
                    @dblclick="
                      editCell(track.id, 'musicalKey', track.musicalKey)
                    "
                  >
                    {{ track.musicalKey }}
                  </span>
                ...
                >
                  <span
                    ...
                    class="cursor-pointer hover:underline"
                    @dblclick="
                      editCell(track.id, 'durationSecs', track.durationSecs)
                    "
                  >
                    {{ Math.floor(track.durationSecs / 60) }}:{{
                      String(track.durationSecs % 60).padStart(2, '0')
                    }}
                  </span>
                  <span v-else class="text-gray-500">—</span>
                </td>
                  ...
                  >
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth="2"
                        d="M12 14l2-2-5 5 2 2 2 2-2-2 5 5-2-2z"
                      />
                    </svg>
                    <template v-else>
                ...
                <td
                  v-if="warnings.value.length > 0"
                  class="py-4 pl-4 text-sm text-yellow-400"
                >
                  {{
        ...
        <div v-if="warnings.value.length > 0" class="mt-4">
          ...
          <div
            class="max-h-24 overflow-y-auto border border-gray-700 rounded bg-gray-800"
          >
        ...
        <div
          v-if="tracks.value.some((t) => t.artworkStatus === 'pending')"
          ...
          <div class="flex items-center gap-2">
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                strokeWidth="2"
                d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.14 16c-.77 1.333.192 3 1.732 3z"
              />
            </svg>
            <span>
              Some track artwork is still loading.
              <button
                @click="startArtworkPolling"
                class="ml-2 underline hover:text-yellow-200"
              >
                Retry
              </button>
            </span>
      ...
      <div class="flex-1 flex flex-col gap-6">
        ...
        <div class="bg-gray-800 rounded-lg p-4">
          ...
          <div class="mb-4">
            ...
            <div class="space-y-2">
              <!-- Solid -->
              <label
                class="flex items-center gap-3 cursor-pointer p-3 rounded hover:bg-gray-700/50"
          ...
          <div v-if="tracks.value.length > maxTracks.value" class="mb-4">
            ...
            <p class="mt-1 text-xs text-gray-500">
              Showing {{ (trackRangeStart.value || 0) + 1 }} -
              {{ (trackRangeEnd.value || tracks.value.length - 1) + 1 }} of
          ...
          <div class="mb-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1 space-y-2">
                ...
                <div v-else class="text-xs text-gray-500">No logo selected</div>
              </div>
            </div>
          ...
          <div class="mb-4">
            <label class="block text-sm font-medium mb-2"
              >Custom Placeholder Image</label
            >
            <div class="flex items-center gap-3">
              <input
                type="file"
              ...
              <div class="flex-1 space-y-2">
                <div
                  v-if="customPlaceholderPath.value"
                  class="flex items-center gap-2"
                >
                  <img
                    :src="customPlaceholderPath.value"
                    alt="Placeholder"
                    class="h-10 w-auto"
                  />
                  <button
        ...
        <div class="flex-1 bg-gray-800 rounded-lg p-4 flex flex-col">
          ...
          <div
            v-if="exportError"
            class="mt-2 p-3 bg-red-900 rounded text-red-300 text-sm"
          >
            {{ exportError }}
          </div>
        </div>
    ...
    <div v-if="pastTracklists.value.length > 0" class="mt-8">
      ...
      <div
        ...
        class="p-4 bg-red-900 rounded text-red-300"
      >
        {{ pastTracklistsError }}
      </div>
